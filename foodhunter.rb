...
class Food
 ...
 def move
  ...
  def update_direction
    if (rand(200) == 0 and !@direction)
    @direction = true
    @smoke = Smoke.new
    @origin= Gosu.milliseconds
  end
  end

  def random_direction
    if (@direction and (Gosu.milliseconds - @origin).abs > 700)
      @vel_x = rand(-2 .. 2)
      @vel_y = rand(-2 .. 2)
      @direction = false
   end
  ...
  if (@direction and (Gosu.milliseconds - @origin).abs < 800)
    @smoke.draw(@x, @y, @angle)
  end
  end
  ...
  def draw(x,y,angle)
  @smoke.draw_rot(x,y,ZOrder::UI,angle)
...
class FoodHunterGame < (Example rescue Gosu::Window)
  ...
  def update
    ...
    @food.each { |food| 
	food.move 
	food.update_direction
	food.random_direction
	}
